    ...
    Class ClassUniqueDropsStats {
        ...
        _setUniqueDropsDryStreak() {
            loop % this.uniqueDrops.length() {
                    ...
                    loop % kills.length() {
                        ...
                        loop % drops.length() {
                            ...
                            If (drop.name = uniqueDrop.name) and (drop.quantity = uniqueDrop.quantity) {
                                If (previousDrop.name = uniqueDrop.name) and (previousDrop.quantity = uniqueDrop.quantity)
                                    dryKills := 0
                                
                                dryKillstreaks[dryKills] := uniqueDrop.name
                                dryKills := 0
                            }
                ...
                ; add trailing kills to the drystreaks, aka kills after the last drop was found
                If (dryKills > 1) ; <- otherwise it counts the trailing dryKills++ and it will register it drop as having a 0 dry record
                    dryKillstreaks[dryKills-1] := uniqueDrop.name


                ; -1 to account for the fact that the dry kills are counted after the loop
                this.uniqueDrops[uniqueDropIndex].dryStreak := dryKills-1 ; CURRENT dry streak
                this.uniqueDrops[uniqueDropIndex].dryStreakRecordLow := dryKillstreaks.MinIndex()
                this.uniqueDrops[uniqueDropIndex].dryStreakRecordHigh := dryKillstreaks.MaxIndex()
            }
        }
    }
}